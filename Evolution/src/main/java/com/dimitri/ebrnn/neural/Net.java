...
public class Net {
	...
	/**
	 * The smoothing factor of which the average error rate is calculated. This lies between 0 and
	 * 1.
	 */
	 ...
	 *
	 * @param topology The layout of the net. <br> Each int represents the amount of neurons in that
	 *                 particular layer.
	 */
	...
	/**
	 * Constructor which takes in a String as a filePath. <br> This constructor uses the file as a
	 * database for the weights.
	 *
	...
	public double[][][][] getWeights() {
		...
		for (int layer = 1; layer < getLayer().length; layer++) {
			...
			for (int neuron = 0; neuron < getLayer(layer).getCell().length; neuron++) {
				...
				for (int gate = 0; gate < 8; gate++) {
					if (gate % 2 == 0) {
						weights[layer][neuron][gate] = new double[((LSTMCell) getLayer(layer)
								.getCell(neuron)).getConnection().length];
						for (int weight = 0; weight < ((LSTMCell) getLayer(layer).getCell(neuron))
								.getConnection().length; weight++) {
							if (gate == 0) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getConnection(weight).getWeight();
							} else if (gate == 2) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getInputGate().getConnection(weight)
														 .getWeight();
							} else if (gate == 4) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getForgetGate().getConnection(weight)
														 .getWeight();
							} else if (gate == 6) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getOutputGate().getConnection(weight)
														 .getWeight();
							}
						}
					} else {
						weights[layer][neuron][gate] = new double[((LSTMCell) getLayer(layer)
								.getCell(neuron)).getRecurrentConnection().length];
						for (int weight = 0; weight < ((LSTMCell) getLayer(layer).getCell(neuron))
								.getRecurrentConnection().length; weight++) {
							if (gate == 1) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getRecurrentConnection(weight)
														 .getWeight();
							} else if (gate == 3) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getInputGate()
														 .getRecurrentConnection(weight)
														 .getWeight();
							} else if (gate == 5) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getForgetGate()
														 .getRecurrentConnection(weight)
														 .getWeight();
							} else if (gate == 7) {
								weights[layer][neuron][gate][weight] = ((LSTMCell) getLayer(layer)
										.getCell(neuron)).getOutputGate()
														 .getRecurrentConnection(weight)
														 .getWeight();
							}
						}
	...
	/**
	 * This method feeds the input array into the input cells. <br> It then feeds this forward
	 * through the network.
	 *
