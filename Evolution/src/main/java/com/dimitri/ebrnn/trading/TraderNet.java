	...
	public void feed(ArrayList<TickerDataParser> array) {
		...
		for (TickerDataParser t : array) {
			...
			for (int i = 0; i < prices.length && i < lows.length && i < highs.length; i++) {
				...
				if (i > 200) {
					Probability probabilityLength = new Probability(new StandardDeviation(
							macd.getBowList().getBowLength()));
					Probability probabilityHeight = new Probability(new StandardDeviation(
							macd.getBowList().getBowHighest()));
					Probability probabilityHighestPlace = new Probability(new StandardDeviation(
							macd.getBowList().getBowHighestPlace()));
					...
					double rising = 0;
					if (macd.getCurrentBow().isPositive()) { rising = 1; }
					double currentFibbonacci = fibonacci.getProjectedProfit();
					double buy = 0;
					if (fibonacci.isBuy()) { buy = 1; }
